...
def generate(grammar,picture):
    ...
    while x < 2 :
        ...
        for m in range(len(store_pic)):
            ...
            for i in range(len(store_pic)):
                check = len(store_pic[i][0])
                for _ in range(len(store_pic[i])):
                    if len(store_pic[i][_]) != check:
                        remove_string.append(store_pic[i])
            new_store_pic.append([x for x in store_pic if x not in remove_string])
            print(new_store_pic)
    ...
    return
    while len(store_pic) != 0 :
        for m in range(len(store_pic)):
            min_col = len(store_pic[m][0])
            min_row = len(store_pic[m])

            if min_col <= pic_col_size[0]:
                # now apply each col table and store it in store_pic
                for _ in range(len(col_dict)):
                    # print(col_dict[_])
                    # print_picture(apply_col_tab(col_dict[_],array_Temp))
                    min_col = len(apply_col_tab(col_dict[_],store_pic[m])[0])
                    min_row = len(apply_col_tab(col_dict[_],store_pic[m]))
                    if min_col >  len(store_pic[m][0]) or min_row > len(store_pic[m]):
                        store_pic_next.append(apply_col_tab(col_dict[_],store_pic[m]))
            # THen do the same for row:
            if min_row <= pic_row_size:
                for _ in range(len(row_dict)):
                    # print(row_dict[_])
                    min_col = len(apply_col_tab(row_dict[_],list(map(list, zip(*store_pic[m]))))[0])
                    min_row = len(apply_col_tab(row_dict[_],list(map(list, zip(*store_pic[m])))))
                    if min_col >  len(store_pic[m][0]) or min_row > len(store_pic[m]):
                        Temp = apply_col_tab(row_dict[_],list(map(list, zip(*store_pic[m]))))
                        # print_picture(list(map(list, zip(*Temp))))
                        store_pic_next.append(list(map(list, zip(*Temp))))
        store_pic = deepcopy(store_pic_next)
        store_pic_next.clear()
        ...
        for i in range(len(store_pic)):
            # print(store_pic[i], i)
            min_col = len(store_pic[i][0])
            min_row = len(store_pic[i])
            for _ in range(len(store_pic[i])):
                if min_col != len(store_pic[i][_]):
                    remove_string.append(store_pic)
            if min_col > pic_col_size[0] or min_row > pic_row_size:
                remove_string.append(store_pic[i])
        store_pic = [x for x in store_pic if x not in remove_string]
        print('AAAAAAAAAAAAAAAAAAAAAAA'*10)
        for i in range(len(store_pic)):
            print(store_pic[i])
        print('AAAAAAAAAAAAAAAAAAAAAAAAAA'*10)
    # Check what's remaining
    # for i in range(len(store_pic)):
    #     print(store_pic[i])
    #     print(len(store_pic[i][0]),len(store_pic[i]))
    print(chk_pic)
    print(len(chk_pic[0]),len(chk_pic))
    print(count_ways)
